...
`newPlotSelect()`, `PCALandscape()`, `meanminSelect()` and `minimaxSelect()` also work with non-spatial data, as a dataframe or matrix.
...
`newPlotSelect()` gives similar results using either a raster or a dataframe of structural metrics. However, when using a dataframe of structural metrics there is no option to specify the dimensions of the selected plots. The function only returns indices of the selected rows from the dataframe of structural metrics.
