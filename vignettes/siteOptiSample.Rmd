...
# Import stem data from existing plots
data(san_lorenzo_stems)
...
set.seed(9238)  # Set identical seed to ensure identical results
test_df3 <- plotSelect(
  r = r_df_all,
  p = p,
  n_plots = 10,
...
`plotSelect()` also has argument `het_q`, a numeric value between 0 and 1 defining a threshold to filter out internally heterogeneous candidate plot locations. This prevents plots being placed on sharp structural transitions. `het_q` represents the maximum quantile for the total focal standard deviation of metrics within candidate plot footprint, e.g. 0.8 excludes the top 20% most heterogeneous areas. 
...
# Part 4: Selecting a subset of existing plots for additional data collection

In this example the goal is to pick 1 ha subplots within a larger plot to
collect terrestrial LiDAR data. The criteria for picking these plots is that
they should span a range of above-ground biomass density, and at least one of
the selected subplots should include some of the largest trees in the plot. 

In this example, the data are entirely non-spatial. No spatial aggregation is performed.
...
plot_metrics <- s %>%
  group_by(plot_id) %>%
  summarise(
    total_agb = sum(agb_Mg),
    max_diam = max(diam_cm),
    sd_diam = sd(diam_cm)
  ) %>%
  ungroup()

r_df_agb <- as.data.frame(
  plot_metrics[, c("total_agb", "max_diam", "sd_diam"), drop = FALSE])
