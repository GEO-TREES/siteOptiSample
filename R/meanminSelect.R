...
meanminSelect <- function(r_pca, p_pca, old_ind, new_ind, n_plots, p_new_dim) { 
  ...
  if (!inherits(r_pca, "SpatRaster")) {
    ...
    for (i in seq_len(n_plots)) { 
      ...
      # Initialize minimum distances from each point to the closest existing plot
      min_dists <- rep(Inf, nrow(r_mat))
      if (!is.null(current_p_mat) && nrow(current_p_mat) > 0) {
        min_dists <- apply(as.matrix(proxy::dist(r_mat, current_p_mat)), 1, min)
      }
      
      # Calculate reduction of mean distance for each candidate
      mean_after <- sapply(new_ind, function(j) {
        cand_vec <- r_mat[j, ]
        cand_mat <- matrix(cand_vec, nrow = nrow(r_mat), ncol = ncol(r_mat), byrow = TRUE)
        new_min <- pmin(min_dists, sqrt(rowSums((r_mat - cand_mat)^2)))
        mean(new_min, na.rm = TRUE) 
      })
      
      # Pick the candidate that results in the lowest mean minimum distance
